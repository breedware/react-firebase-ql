...
export class BaseModel implements Model {
     ...
     * @param id 
     */
    stream(callBack: (data:  DocumentData | DocumentData[] | undefined)=>void, id?: string ) { 
        ...
        try {
            if(id){
                return onSnapshot(ref as DocumentReference, (doc) => {
                    callBack(doc.exists() ? {...doc.data(), reference: ref.id}: undefined)
                })
            }else{
                return onSnapshot(ref as CollectionReference, (snapShot) => {
                    callBack(
     ...
     * @returns {boolean}
     */
    async updateAtomicArray(data: any[], reference: string, key: string): Promise<boolean> {
        try {
